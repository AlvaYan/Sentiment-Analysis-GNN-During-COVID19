    ...
    for split in split_list:
        ...
        if time:
            for i in range(time):
                ...
                classification_rate=sum(y_pred==test_y)/len(test_y)
...
#                time, ss, k, sum(macro_list) / len(macro_list), sum(micro_list) / len(micro_list)))
            print('KNN({}avg, split:{}, k={}) classification_rate: {:.4f}, f1_micro: {:.4f}'.format(
                time, ss, k, classification_rate, sum(micro_list) / len(micro_list)))
